...
class Student {
    ...
    public constructor(_name: string, _matrikelnummer: number, _studiengang: string, _semester: number) {
        this.name = _name;
...
let student1: Student = new Student("Sören Winterhalder", 266619, "Medieninformatik", 2);
let student2: Student = new Student("Max Mustermann", 123456, "Musterstudiengang", 3);
let student3: Student = new Student("Mira Musterfrau", 654321, "Musterstudiengang", 4);

let studentenArray: Student[] = [student1, student2, student3, new Student("Irgend Jemans", 162534, "Irgendeinstudiengang", 1)];
...
function split(_arr: number[], _index1: number, _index2: number): number[] {
    ...
    let start: number = (_index1 < _index2) ? _index1 : _index2;
    let end: number = (_index1 > _index2) ? _index1 : _index2;

    //Passt den start/end index an falls dieser nicht im array liegt
    if (start < 0) start = 0;
    if (end > _arr.length - 1) end = _arr.length - 1;
...
let objectContext: CanvasRenderingContext2D = objectCanvas.getContext("2d");

class DrawingObject {
    public x: number;
    public y: number;
    public scale: number = 2; //je größer desto kleiner die Objekte
